...
import com.dagou.cnn.DataSet.Record;
import com.dagou.cnn.Layer.Size;
import com.dagou.cnn.util.ConcurentRunner.TaskManager;
import com.dagou.cnn.util.MathUtils;
import com.dagou.cnn.util.MathUtils.Operator;
...
public class CNN implements Serializable {
	...
	public void train(DataSet trainset, int repeat) {
		...
		for (int t = 0; t < repeat && !stopTrain.get(); t++) {
			...
			for (int i = 0; i < epochsNum; i++) {
				int[] randPerm = MathUtils.randomPerm(trainset.size(),
						batchSize);
	...
	public String predict(DataSet testset) {
		logger.info("开始预测 ...");
				...
				for (int j = start; j < end; j++) {
					for (int i = 0; i < lastMapNum; i++) {
						...
						for (int r = 0; r < batchSize; r++) {
							...
							if (deltaKernel == null)
								deltaKernel = MathUtils.convnValid(
										lastLayer.getMap(r, i), error);
							else {
								deltaKernel = MathUtils
										.matrixOp(MathUtils.convnValid(
												lastLayer.getMap(r, i), error),
												deltaKernel, null, null,
												MathUtils.plus);
							}
						}

						deltaKernel = MathUtils.matrixOp(deltaKernel,
								divide_batchSize);
						double[][] kernel = layer.getKernel(i, j);
						deltaKernel = MathUtils
								.matrixOp(kernel, deltaKernel, multiply_lambda,
										multiply_alpha, MathUtils.plus);
			...
			public void process(int start, int end) {
				for (int i = start; i < end; i++) {
					...
					for (int j = 0; j < nextMapNum; j++) {
						...
						if (sum == null)
							sum = MathUtils.convnFull(nextError,
									MathUtils.rot180(kernel));
						else
							sum = MathUtils.matrixOp(
									MathUtils.convnFull(nextError,
											MathUtils.rot180(kernel)), sum,
		...
		new TaskManager(mapNum) {
			...
			public void process(int start, int end) {
				for (int m = start; m < end; m++) {
					...
					double[][] outMatrix = MathUtils.matrixOp(map,
							MathUtils.cloneMatrix(map), null,
							MathUtils.one_value, MathUtils.multiply);
					outMatrix = MathUtils.matrixOp(outMatrix,
							MathUtils.kronecker(nextError, scale), null, null,
							MathUtils.multiply);
				...
				for (int j = start; j < end; j++) {
					...
					for (int i = 0; i < lastMapNum; i++) {
						...
						if (sum == null)
							...
							sum = MathUtils.matrixOp(
									MathUtils.convnValid(lastMap, kernel), sum,
									null, null, MathUtils.plus);
					}
		...
		new TaskManager(lastMapNum) {
			...
			public void process(int start, int end) {
				for (int i = start; i < end; i++) {
					...
					double[][] sampMatrix = MathUtils.scaleMatrix(lastMap,
							scaleSize);
